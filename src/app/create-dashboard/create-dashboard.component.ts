...
export class CreateDashboardComponent implements AfterViewInit {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        if (result.event === 'save') {
          console.log(result);
          this.createChartDetails.push({
            reportId: result.data.id,
            chartName: result.data.name,
          });
          console.log(this.createChartDetails);
          this.add(
            this.createChartDetails[this.createChartDetails.length - 1],
            this.createChartDetails.length,
            ''
          );
        }
      }
  ...
  private loadCharts() {
    if (this.createChartDetails!=null && this.createChartDetails.length == 0) {
  ...
  private add(data: any, index: number, status: string) {
    ...
    let contentElement: HTMLElement = newWidget.querySelector(
  ...
  async getDashboardDetails() {
    ...
    this.createChartDetails = await firstValueFrom(
